...
export function Terminal() {
  ...
  return <div className="terminal">
    ...
    <div className="terminal-log" ref={log} role="log" aria-live="polite" aria-relevant="additions text">
      ...
      {commands.map((command, index) => <div className="terminal-entry" key={index}><p><span className="terminal-prompt">{siteText.terminal.prompt}</span> {command}</p><pre>{runCommand(command, language)}</pre></div>)}
    </div>
